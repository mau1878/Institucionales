...
st.write(f"Debug: Page title set to 'Rankings de Mercado'")
...
try:
    ...
    new_positions_df = merged_data[merged_data['Shares Change % num'].isna() | np.isinf(merged_data['Shares Change % num'].fillna(np.inf))]
    ...
    st.error(f"Error en la página de Rankings de Mercado: {str(e)}")